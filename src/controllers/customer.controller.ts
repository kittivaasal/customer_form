...
export const createCustomer = async (req: CustomRequest, res: Response) => {
  ...
  let { introducerId, marketerDetailId, ddId, cedId, email, phone, projectId } =
    body;
  ...
  if (isNull(ddId) && isNull(cedId)) {
    return ReE(
      res,
      { message: `Please enter ddId or cedId!.` },
  ...
  if (!isNull(ddId)) {
    ...
    if (!findDd) {
      return ReE(
        res,
        { message: `Dd is not found for given id!.` },
        httpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  if (!isNull(introducerId)) {
    if (!mongoose.isValidObjectId(introducerId)) {
      return ReE(
        res,
        { message: `Invalid introducerId!.` },
        httpStatus.BAD_REQUEST,
      );
    }
    let findIntroducer;
    [err, findIntroducer] = await toAwait(
      MarketingHead.findOne({ _id: introducerId }),
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
    if (!findIntroducer) {
      return ReE(
        res,
        { message: `Introducer is not found for given id!.` },
        httpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  if (!projectData) {
    return ReE(
      res,
      { message: `Project not found for given project id!.` },
  ...
  if (projectData?.projectName) {
    let id = projectData?.shortName
      ? projectData?.shortName.endsWith("-")
        ? projectData?.shortName
        : projectData?.shortName + "-"
      : toAutoIncrCode(projectData?.projectName);
    let getCustomerCounter,
      count = 0;
    [err, getCustomerCounter] = await toAwait(
      Counter.findOne({ name: "customerid" }),
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
  ...
  if (req.query.includeCustomer === "true") {
    return ReS(
      res,
      { message: `customer added successfull`, data: customer },
      httpStatus.CREATED,
    );
  }
...
export const updateCustomer = async (req: CustomRequest, res: Response) => {
  ...
  if (!isNull(updateFields.cedId)) {
    ...
    let findCed;
    [err, findCed] = await toAwait(
      MarketDetail.findOne({ _id: updateFields.cedId }),
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
    if (!findCed) {
      return ReE(
        res,
        { message: `Ced is not found for given id in market detail table!.` },
        httpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  if (!isNull(updateFields.ddId)) {
    ...
    let findDd;
    [err, findDd] = await toAwait(
      MarketingHead.findOne({ _id: updateFields.ddId }),
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
    if (!findDd) {
      return ReE(
        res,
        { message: `Dd is not found for given id in marketing head table!.` },
        httpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  if (!isNull(updateFields.introducerId)) {
    ...
    let findIntroducer;
    [err, findIntroducer] = await toAwait(
      MarketingHead.findOne({ _id: updateFields.introducerId }),
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
    if (!findIntroducer) {
      return ReE(
  ...
  if (updateFields.projectId) {
    ...
    let findProject;
    [err, findProject] = await toAwait(
      Project.findOne({ _id: updateFields.projectId }),
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
    if (!findProject) {
      return ReE(
  ...
  if (getEditRequest) {
    return ReE(
      res,
      {
        message: `Already this customer has pending edit request so you can't update!`,
      },
      httpStatus.BAD_REQUEST,
    );
  }
  ...
  let getGen;
  [err, getGen] = await toAwait(General.findOne({ customer: _id }));
  if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
  if (!getGen)
    return ReE(res, { message: `Ced not found!` }, httpStatus.NOT_FOUND);
  ...
  if (updateFields.cedId || updateFields.ddId || updateFields.projectId) {
    ...
    if (!isEmpty(object)) {
      [err, update] = await toAwait(
        General.updateOne({ customer: _id }, { $set: object }),
      );
      if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
      if (!update) {
        return ReE(
          res,
          { message: `Failed to update marketer in general!` },
          httpStatus.NOT_FOUND,
        );
      }
    }
...
export const getByIdCustomer = async (req: Request, res: Response) => {
  ...
  let { general, plot } = req.query;

  let getGeneral;
  if (general === "true") {
    [err, getGeneral] = await toAwait(
      General.findOne({ customer: id }).populate("project"),
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
    if (!getGeneral) {
      return ReE(
  ...
  let getPlot = null;
  if (plot === "true") {
    let getGeneralForPlot;
    [err, getGeneralForPlot] = await toAwait(General.findOne({ customer: id }));
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
    if (getGeneralForPlot) {
      [err, getPlot] = await toAwait(
        Plot.findOne({ general: (getGeneralForPlot as any)._id }),
      );
      if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
      // If no plot found, getPlot stays null — no error thrown
    }
  }
...
export const deleteCustomer = async (req: CustomRequest, res: Response) => {
  ...
  if (!_id) {
    return ReE(
      res,
      { message: `Customer _id is required!` },
      httpStatus.BAD_REQUEST,
    );
  }
  if (!mongoose.isValidObjectId(_id)) {
  ...
  if (!user.isAdmin) {
    ...
    if(checkBillingRequest) {
      return ReE(
        res,
        { message: "Customer delete request already pending for this billing id!" },
        httpStatus.BAD_REQUEST,
      );
    }
    let getAllEmi;
